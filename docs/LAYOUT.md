...
The front silkscreen also includes the project-local `G1` coffee-cup and
weather-balloon artwork centered at (140,164) mm. Its 7.50 x 11.25 mm
monochrome footprint occupies the clear lower-center strip below the LightAPRS
body and between the two SMA footprints. The nearby `U2 DRIVER - LEFT DIP`
caption is at y=157 mm so the caption and artwork remain separately readable.
The preserved source image and printable conversion are in
`assets/silkscreen/`; the footprint is reproducibly generated by
`scripts/generate_silkscreen_logo.py`.
