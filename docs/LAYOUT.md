...
- Implement and verify the approved split-power architecture in a later hardware pass: S7V8F5 item 2123 VIN and SHDN on `PACK_SW`, GND common, VOUT=`LOGGER_5V`, and A1 VCC on `LOGGER_5V`, while LightAPRS RAW/J2.1 and J5.1 remain direct `PACK_SW`. Qualify OpenLog ≤7 mA idle and ≤25 mA write, regulator Iq <0.2 mA, board-added ≤8 mA idle and ≤30 mA active peak, U1 ≤100 µA, reverse-polarity handling, and cold operation.
